...
- Forecast data loads from `/api/nws-forecast-detailed` with fallback to `http://132.145.154.22:3000/api/nws-forecast-detailed`.
- Current conditions load from `/api/tempest/current` with fallback to `http://132.145.154.22:3000/api/tempest/current`.
- Alert data loads from `/api/nws-alerts` with fallback to `http://132.145.154.22:3000/api/nws-alerts`.
...
## GitHub Pages Oracle mode
This copy is preconfigured to call the Oracle backend at `http://132.145.154.22:3000`.
